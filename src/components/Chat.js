...
const Chat = (props) => {
  ...
  const getMessages = () => {
    axios
      .post('https://chat-app-backend.herokuapp.com/api/allmessages', {
        my_id: props.user.id,
        other_id: Number(targetid),
      })
  ...
  const addMessage = () => {
    axios
      .post('https://chat-app-backend.herokuapp.com/api/sendmessage', {
        user1: props.user.id,
        user2: Number(targetid),
